...
export type UpdateLabelData = Partial<Omit<CreateLabelData, 'sId' | 'randId' | 'createdAt'>>;
...
export class LabelRepository {
  ...
  static async findByRandId(randId: string): Promise<LabelData | null> {
    const [label] = await db.select().from(labelModel).where(eq(labelModel.randId, randId));
    return label || null;
  }
  ...
  static async searchByName(searchTerm: string, limit: number = 10): Promise<LabelData[]> {
    return await db.select().from(labelModel)
      .where(eq(labelModel.name, searchTerm))
      .limit(limit);
  }

  // Update label by sequential ID
  static async updateById(sId: number, data: UpdateLabelData): Promise<LabelData | null> {
    const [label] = await db.update(labelModel)
      .set(data)
      .where(eq(labelModel.sId, sId))
      .returning();
    return label || null;
  }

  // Update label by random UUID
  static async updateByRandId(randId: string, data: UpdateLabelData): Promise<LabelData | null> {
    const [label] = await db.update(labelModel)
      .set(data)
      .where(eq(labelModel.randId, randId))
      .returning();
    return label || null;
  }
