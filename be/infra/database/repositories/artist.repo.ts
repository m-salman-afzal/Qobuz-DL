...
export type UpdateArtistData = Partial<Omit<CreateArtistData, 'sId' | 'randId' | 'createdAt'>>;
...
export class ArtistRepository {
  ...
  static async findByRandId(randId: string): Promise<ArtistData | null> {
    const [artist] = await db.select().from(artistModel).where(eq(artistModel.randId, randId));
    return artist || null;
  }
  ...
  static async searchByName(searchTerm: string, limit: number = 10): Promise<ArtistData[]> {
    return await db.select().from(artistModel)
  ...
  static async updateById(sId: number, data: UpdateArtistData): Promise<ArtistData | null> {
    const [artist] = await db.update(artistModel)
      .set(data)
      .where(eq(artistModel.sId, sId))
      .returning();
    return artist || null;
  }

  // Update artist by random UUID
  static async updateByRandId(randId: string, data: UpdateArtistData): Promise<ArtistData | null> {
    const [artist] = await db.update(artistModel)
      .set(data)
      .where(eq(artistModel.randId, randId))
      .returning();
    return artist || null;
  }
