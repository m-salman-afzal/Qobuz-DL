...
export type UpdateGenreData = Partial<Omit<CreateGenreData, 'sId' | 'randId' | 'createdAt'>>;
...
export class GenreRepository {
  ...
  static async findByRandId(randId: string): Promise<GenreData | null> {
    const [genre] = await db.select().from(genreModel).where(eq(genreModel.randId, randId));
    return genre || null;
  }
  ...
  static async searchByName(searchTerm: string, limit: number = 10): Promise<GenreData[]> {
    return await db.select().from(genreModel)
      .where(eq(genreModel.name, searchTerm))
      .limit(limit);
  }
  ...
  static async findByPath(path: number[]): Promise<GenreData[]> {
    return await db.select().from(genreModel)
      .where(eq(genreModel.path, path));
  }

  // Update genre by sequential ID
  static async updateById(sId: number, data: UpdateGenreData): Promise<GenreData | null> {
    const [genre] = await db.update(genreModel)
      .set(data)
      .where(eq(genreModel.sId, sId))
      .returning();
    return genre || null;
  }

  // Update genre by random UUID
  static async updateByRandId(randId: string, data: UpdateGenreData): Promise<GenreData | null> {
    const [genre] = await db.update(genreModel)
      .set(data)
      .where(eq(genreModel.randId, randId))
      .returning();
    return genre || null;
  }
